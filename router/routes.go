...
func AddRoutes(r runtime.SuperclusterRuntime, s store.P2PStore) {
	/** middleware/config **/
	// cors allow all
	...
	api.POST("/user/:userId/clusters", func(ctx *gin.Context) { getUserClusters(ctx) })
	api.POST("/user/pinFile", func(ctx *gin.Context) { createPin(ctx, s) })

	// cluster API
	api.POST("/cluster", func(ctx *gin.Context) { createCluster(ctx) })
	api.GET("/cluster/:clusterId/files", func(ctx *gin.Context) { listPinnedFiles(ctx, s) })
	api.GET("/cluster/:clusterId/addrs", func(ctx *gin.Context) { listClusterAddrs(ctx, s) })
	...
	// file API
	api.GET("/cluster/:clusterId/:fileCid", func(ctx *gin.Context) { getFile(ctx, s) })
