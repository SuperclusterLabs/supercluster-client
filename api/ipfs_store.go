...
func newIpfsStore() (Store, context.Context, error) {
...
func (s *ipfsStore) Create(ctx context.Context, name, contents string) (*file, error) {
	...
	peerCid, err := s.ipfs.Unixfs().Add(ctx, files.NewBytesFile([]byte(contents)))
	if err != nil {
